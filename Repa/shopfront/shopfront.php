...
<head>
    ...
    <link rel="stylesheet" href="css/shopfront.css">

    <!-- FONTS -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Ubuntu:ital,wght@0,300;0,400;0,500;0,700;1,300;1,400;1,500;1,700&display=swap" rel="stylesheet">
    <title>repa | Vásárlás</title>
</head>
<body>
    ...
    <div class="banner">
        <div class="banner-box">
            <h1>Friss répa szállítása minden reggel</h1>
        <h3>Egyenesen boltod, éttermed vagy házad küszöbéig szállítunk</h3>
        </div>
    </div>
    ...
    <main class="shop">
        ...
        <div class="container">
            ...
            $sql = mysqli_query($conn, "SELECT * FROM `products` WHERE `type`='repa'");

            foreach ($sql as $product) {
                echo'<form action="php/shopfront.inc.php" method="POST" class="product-box">
                    <input type="hidden" value='.$product["id"].' name="pr_id">
                    <img src="../admin/img/'.$product["image"].'" alt="">
                    <h4>'.$product["name"].'</h4>
                    <h5>'.$product["price"].' Ft/kg</h5>
                    <div class="input">
                        <input type="number" placeholder="kg" name="amount">
                        <input type="submit" value="Hozzáadás" name="submit">
                    </div>
                </form>';
            }
            ?>
    ...
    if(isset($_SESSION["hasDiscountCode"]) && $_SESSION["hasDiscountCode"] !== 1) {
        echo '<form id="border" method="post" action="php/shopfront.inc.php">
        <p id="question">Generálj kérdést</p>
        <select name="select" id="answer">
            <option value="selected">Válassz</option>
            <option value="optional1">1</option>
            <option value="optional2">2</option>
            <option value="optional3">3</option>
        </select>
    
        <button onclick="goodOrWrongAlert()" type="submit" name="submitQuiz" id="submit">Küldés</button>
    </form>';
    }
    ?>
    ...
    <div class="overlay" <?php if(isset($_SESSION['showGoodAlert'])) echo 'style="display: block;"'; else echo 'style="display: none;"'; ?>>
        <div class="alert">
            <div class="alertHead">
            <h3>A válasz helyes! Itt a jutalmad!</h3>
                <form action="php/shopfront.inc.php" method="post" class="alertclose-form">
                    <input type="submit" name="alertClose" value="X">
                </form>
            </div>
            <div class="alertBody">
                <h2 id="discountText">
    ...
    <div class="errorOverlay" <?php if(isset($_SESSION['showWrongAlert'])) echo 'style="display: block;"'; else echo 'style="display: none;"'; ?>>
        <div class="alert">
            <div class="alertHead">
                <h3>Sajnos a válasz helytelen!</h3>
                <form action="php/shopfront.inc.php" method="post" class="alertclose-form">
                    <input type="submit" name="alertClose" value="X">
                </form>
            </div>
            <div class="alertBody">
                <h2>Ma nem kapsz kupont :c</h2>
            </div>
        </div>
    </div>
